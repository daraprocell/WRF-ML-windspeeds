...
│   ├── 3D_plots.py               # 3D cold pool visualizations (Plotly htmls)
...
All data is included in the data folder, and can be generated by the download scripts EXCEPT
the wrfout files because they are too large. Those can be found on keeling in my scratch
folder, and can be accessed upon request.
...
- `wrf_refl.gif`, `obs_refl.gif`, `refl_comparison.gif`
